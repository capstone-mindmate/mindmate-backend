...
public class PaymentServiceImplTest {
    ...
    void setUp() {
        ...
        ProfileImage testImage = ProfileImage.builder()
                .imageUrl("test.png")
                .user(testUser)
                .build();
    ...
    void confirmPayment() {
        ...
        verify(paymentOrderRepository, times(1)).findByOrderIdWithLock(orderId);
    ...
    void confirmPaymentWithNonExistingOrder() {
        ...
        verify(paymentOrderRepository, times(1)).findByOrderIdWithLock(anyString());
    ...
    @Test
    @DisplayName("이미 처리된 주문으로 결제 확인 시 성공 응답 반환")
    ...
    @Test
    @DisplayName("READY 상태의 주문 상세 정보 조회 시 영수증 URL 없음")
    ...
    @Test
    @DisplayName("포인트 적립 중 예외 발생 시에도 결제는 성공 처리")
    void confirmPaymentWithPointServiceException() {
        ...
        verify(pointService, times(1)).addPoints(eq(testUser.getId()), any(PointRequest.class));
        verify(paymentOrderRepository, times(1)).save(any(PaymentOrder.class));
